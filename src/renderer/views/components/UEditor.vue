<template>
  <script :id="randomId" name="content" type="text/plain">
    <p style="font-size: 14px;color: #999">{{text}}</p>
  </script>
</template>
  ...
  export default {
    ...
    props: {
      ...
      simpleImageBtn: {
        type: Function,
        default: function () {
          return alert('单个图片上传框')
        }
      },
      isShowSimpleImageBtn: {
        type: Boolean,
        default: false
      },
      imageBtn: {
    ...
    methods: {
      ...
      initEditor () {
        ...
        if (this.scriptTagStatus === 2 && this.instance === null) {
          ...
          this.$nextTick(() => {
            this.instance = window.UE.getEditor(that.randomId, this.ueditorConfig)
            // 绑定事件，当 UEditor 初始化完成后，将编辑器实例通过自定义的 ready 事件交出去
            this.instance.addListener('ready', () => {
              this.$emit('ready', this.instance)
            })
            if( that.isShowSimpleImageBtn ){
              UE.registerUI('SimpleImage', function(editor, uiName) {
                let image = new UE.ui.Button({
                  name: uiName,
                  title: '单个图片上传',
                  cssRules: 'background-position: -381px 0',
                  onclick: that.simpleImageBtn
                })
                return image
              })
            }
            if( that.isShowImageBtn ){
              UE.registerUI('Image', function(editor, uiName) {
                let image = new UE.ui.Button({
                  name: uiName,
                  title: '多张图片上传',
                  cssRules: 'background-position: -726px -77px',
                  onclick: that.imageBtn
                })
                return image
              })
            }
            if(that.isShowEnclosureBtn){
              UE.registerUI('Enclosure', function(editor, uiName) {
                let enclosure = new UE.ui.Button({
                  name: uiName,
                  title: '附件上传',
                  cssRules: 'background-position: -622px -39px',
                  onclick: that.enclosureBtn
                })
                return enclosure
              })
            }
